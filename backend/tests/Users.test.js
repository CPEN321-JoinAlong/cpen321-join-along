...
describe("find unblocked users", () => {
    test("Invalid user ID", async () => {
        ...
        expect(foundUsers.status).toBe(ERROR_CODES.INVALID);
    });
    ...
    test("Success: unblocked users returned", async () => {
        const userStore = new UserStore();
        ...
        User.find.mockResolvedValue(userList);
        let foundUserList = await userStore.findUnblockedUsers(
            "62d50cfb436fbc75c258d9eb"
        );
        expect(foundUserList.status).toBe(ERROR_CODES.SUCCESS);
        expect(JSON.stringify(foundUserList.data)).toBe(
